...
const EditStaffScreen = ({ navigation, route }) => {
  ...
  const [form, setForm] = useState({
    ...
    phone: staff.phone || '',
    email: staff.email || '',
    designation: staff.designation || staff.role || '', // fallback to role for backward compatibility
    salary_type: staff.salary_type || 'monthly',
    salary_amount: staff.salary_amount ? staff.salary_amount.toString() : '',
  });
  ...
  const handleSubmit = () => {
    ...
    // Here you would call your API to update the staff data
    console.log('Staff data to update:', { ...form, id: staff.id });
    navigation.goBack();
  };
